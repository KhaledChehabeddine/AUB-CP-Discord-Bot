...
def description(): return "Displays what commands are available for \">session\""

# ------------------ [ init() ] ------------------ #
    # Iterates over names in "folder" file of "config["session_module"]"
    # Verifies if name is a command by checking if last 3 letters == ".py"
    # Commands added to "available_commands", otherwise skipped
def init():
    for (t1, t2, folder) in os.walk(config['session_module']):
        for item in folder:
            if item[-3:] != '.py': continue
            available_commands[item[:-3]] = importlib.import_module(config['session_module'][2:] + '.' + item[:-3])
